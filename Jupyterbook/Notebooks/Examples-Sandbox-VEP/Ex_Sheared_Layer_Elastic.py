...
uw.function.derivative(stokes.constitutive_model.Parameters.ve_effective_viscosity, stokes.constitutive_model.Parameters.shear_viscosity_0)



stokes.constitutive_model.viscosity.diff(stokes.constitutive_model.Parameters.shear_viscosity_0)

eta0 = sympy.symbols(r"eta_0", cls=sympy.core.function.UndefinedFunction)


expr = stokes.constitutive_model.Parameters.ve_effective_viscosity.value
expr2 = expr.subs(stokes.constitutive_model.Parameters.shear_viscosity_0, eta0)
expr2

expr2.diff(eta0, enumerate=False)
...
nodal_tau_inv2.uw_function = stokes.constitutive_model.viscosity * 2 * stokes.Unknowns.Einv2
...
if uw.mpi.size == 1:
    ...
    arrows0 = pl.add_arrows(velocity_points.points, 
                            velocity_points.point_data["V"], 
                            mag=0.1, opacity=1, show_scalar_bar=False)
...
with stokes.DFDt._nswarm_psi.access():
    print(stokes.DFDt._nswarm_psi.swarmVariable.data)

stokes.Unknowns.DFDt._psi_star_projection_solver.solve(verbose=True)

stokes.Unknowns.DFDt._psi_star_projection_solver.solve(verbose=True)

stokes.Unknowns.DFDt.psi_star[0]
